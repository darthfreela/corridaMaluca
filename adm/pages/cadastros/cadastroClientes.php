							...
							<?php
								if (array_key_exists("id", $_GET)) {
									if (is_numeric($_GET["id"])) {
										$connect->conectar();
										$sqlForm = "SELECT cli.clientes_id AS id, 
															cli.nome AS nome,
							...
							<form action="cadastroClientes.php" method="post">
								...
								<div class="box-body">
									...
									<div class="form-group col-md-3">
										...
										<input type="Tel" class="form-control" id="txtTelefone" name="txtTelefone" placeholder="Telefone" required value="<?php echo $telefone; ?>">
									</div>
									...
									<div class="form-group col-md-4">
										...
										<select class="form-control" id="cbEstado" name="cbEstado" placeholder="Estado" required>
											<?php
												$connect->conectar();
												$sqlEstado = "SELECT idEstado AS id,
																		nome AS nome,
																		uf AS uf
																FROM estados 
																ORDER BY nome ASC";
												$connect->set("sql", $sqlEstado);
												$lista = $connect->executar();
												while ($estados = mysql_fetch_object($lista)) {
											?>
											<option value="<?php echo $estados->id; ?>">
												<?php echo $estados->nome; ?>
											</option>
											<?php
												}
											?>
									...
									<div class="form-group col-md-4">
										...
										<select class="form-control" class="form-control" id="cbCidade" name="cbCidade" placeholder="Cidade" required>
											<?php
												$connect->conectar();
												$sqlEstado = "SELECT idCidade AS id,
																		nome AS nome,
																		idEstado AS id
																FROM cidades 
																ORDER BY nome ASC";
